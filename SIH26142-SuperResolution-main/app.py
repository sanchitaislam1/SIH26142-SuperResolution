...
def process_geotiff(file_path, progress=gr.Progress()):
    if file_path is None:
        raise gr.Error("Please upload a Sentinel-2 GeoTIFF.")

    try:
        # ----------------------------------------------------
        # READ AND ROUTE INPUT TIFF
        # ----------------------------------------------------
        progress(0.02, desc="Reading GeoTIFF...")
        ...
        progress(0.05, desc="Preparing input bands...")
        lr_np = prepare_bands(lr_raw)

        # Fallback padding: If < 4 bands, duplicate the available bands across 4 channels 
        # to prevent Gradio UI index errors.
        if band_count != 4:
        ...
        # ----------------------------------------------------
        # LOGIC ROUTER: AI vs MATH SCALER
        # ----------------------------------------------------
        if band_count == 4 and model is not None:
            progress(0.08, desc="Primary Route: Running EDSR AI super-resolution...")
            sr_np = run_tiled_inference(model, lr_np, progress)
            
        else:
            progress(0.08, desc=f"Fallback Route: {band_count}-band detected. Running math scaler...")
            # Use PyTorch's native bicubic interpolation to bypass the AI model
            tensor = torch.from_numpy(lr_np).unsqueeze(0)
            sr_tensor = F.interpolate(tensor, scale_factor=SCALE, mode='bicubic', align_corners=False)
            sr_np = np.clip(sr_tensor.squeeze(0).numpy(), 0.0, 1.0)
        ...
        # CREATE OUTPUT GEOTIFF
        # ----------------------------------------------------
        progress(0.95, desc="Creating 4× GeoTIFF...")
        ...
        if os.path.exists(METRICS_FILE):
            try:
                ...
                metrics = (
                    f"**Average PSNR:** {ep:.2f} dB (+{ep - bp:.2f} dB vs Bicubic)\n\n"
                    f"**Average SSIM:** {es:.4f} (+{es - bs:.4f} vs Bicubic)\n\n"
                    f"**Average MSE:** {em:.6f} ({bm - em:.6f} lower than Bicubic)\n\n"
                    "Benchmark: 541-image test dataset; not metrics for the uploaded image."
                )
            except Exception as e:
        ...
        return (
            lr_rgb, bicubic_rgb, sr_rgb,
            display_band(sr_np[0]), display_band(sr_np[1]),
            display_band(sr_np[2]), display_band(sr_np[3]),
            metrics, info, output_path
        )
