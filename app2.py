...
def create_rgb(image):
    ...
    # Four-band Sentinel-2 mode:
    # B04 Red, B03 Green, B02 Blue, B08 NIR
    if image.shape[0] < 3:
        raise ValueError(
            "RGB preview requires at least 3 bands."
        )
...
def create_preview(image):
    ...
    # 4-band input: display natural RGB using B04/B03/B02.
    if image.shape[0] == 4:
        return create_rgb(image)

    # 1-band input: display the band as a grayscale image.
    if image.shape[0] == 1:
        return display_band(image[0])

    raise ValueError(
        "GeoResolve supports either 1 band or 4 bands."
    )
...
def run_bicubic_preview(image):

    """
    Create a visual bicubic baseline.

    Four-band mode uses RGB only for display.
    Single-band mode interpolates that one band directly.
    """
    ...
    if image.shape[0] == 1:
        return normalize_for_display(result[0])
    ...
    try:
        ...
        with MemoryFile(
            ...
            with memfile.open() as src:
                ...
                if band_count not in (1, 4):

                    raise gr.Error(
                        "GeoResolve supports either 1 band or 4 bands.\n\n"
                        f"This GeoTIFF contains {band_count} bands.\n\n"
                        "For 4-band Sentinel-2 mode use:\n"
                        "B04 Red • B03 Green • B02 Blue • B08 NIR\n\n"
                        "For single-band mode, one raster band is accepted "
                        "and is processed as a single-band preview/output."
                    )
        ...
        band_count, height, width = lr_np.shape
        ...
        original_preview = create_preview(
            lr_np
        )
        ...
        bicubic_preview = run_bicubic_preview(
            lr_np
        )
        ...
        # The trained EDSR checkpoint has 4 input/output channels.
        # For a single-band file, repeat that band across the four
        # model inputs, then keep the first predicted channel.
        model_input = lr_np
...
"""
        ...
        return (
            ...
            display_band(sr_np[0]) if sr_np.shape[0] >= 1 else None,
            display_band(sr_np[1]) if sr_np.shape[0] >= 2 else None,
            display_band(sr_np[2]) if sr_np.shape[0] >= 3 else None,
            display_band(sr_np[3]) if sr_np.shape[0] >= 4 else None,
...
body.dark-mode .gradio-container h1,
body.dark-mode .gradio-container h2,
body.dark-mode .gradio-container h3,
body.dark-mode .gradio-container h4,
body.dark-mode .gradio-container strong {
    color: #f8fafc !important;
}

body.dark-mode .gradio-container p,
body.dark-mode .gradio-container li,
...
body.dark-mode .info-box blockquote {
    color: #cbd5e1 !important;
...
body.dark-mode .gradio-container input,
body.dark-mode .gradio-container textarea,
body.dark-mode .gradio-container select {
    background: #0f172a !important;
    color: #f8fafc !important;
    border-color: #475569 !important;
}

body.dark-mode .gradio-container label,
body.dark-mode .gradio-container .wrap,
body.dark-mode .gradio-container .prose {
    color: #e5e7eb !important;
}

body.dark-mode .gradio-container .secondary {
    background: #1e293b !important;
    color: #f8fafc !important;
    border-color: #475569 !important;
}

body.dark-mode .gradio-container table {
    color: #e5e7eb !important;
}

body.dark-mode .gradio-container th {
    color: #f8fafc !important;
    background: #1e293b !important;
}

body.dark-mode .gradio-container td {
    color: #cbd5e1 !important;
}

body.dark-mode .gradio-container a {
    color: #5eead4 !important;
}
...
with gr.Blocks(
    ...
    gr.HTML(
        ...
        <div class="hero">
            ...
            <div class="hero-subtitle">
                {APP_TAGLINE}
                Upload a 1-band GeoTIFF or a 4-band Sentinel-2 GeoTIFF and
    ...
    with gr.Row():
        ...
        gr.HTML(
            ...
            <div class="step-card">
                ...
                <p>
                    Add a 1-band GeoTIFF or your 4-band Sentinel-2
                    GeoTIFF.
                </p>
    ...
    gr.Markdown(
        ...
        **Accepted inputs:** 1-band GeoTIFF or 4-band Sentinel-2 GeoTIFF.

        **4-band mode:** B04 Red, B03 Green, B02 Blue and B08 NIR.
        **1-band mode:** any single raster band.
        """
    )
...
| Output | 1-band or 4-band GeoTIFF |

### Supported inputs

- **1-band GeoTIFF** — processed using the existing 4-channel EDSR by repeating the input band across the four model inputs; the first predicted channel is returned.
- **4-band Sentinel-2 GeoTIFF** — **B04 Red, B03 Green, B02 Blue, B08 Near Infrared**.
